...
// 1. Bit-wise Hash: Completely stable and eliminates sin() precision issues
fn hash22(p: vec2<f32>) -> vec2<f32> {
    var uv = bitcast<vec2<u32>>(p);
    uv = uv * 1664525u + 1013904223u;
    uv.x += uv.y * 1103515245u;
    uv.y += uv.x * 1103515245u;
    uv = uv ^ (uv >> vec2<u32>(16u));
    uv.x += uv.y * 1103515245u;
    uv.y += uv.x * 1103515245u;
    uv = uv ^ (uv >> vec2<u32>(16u));
    // Convert to float range [0.0, 1.0]
    return vec2<f32>(uv) * (1.0 / f32(0xffffffffu));
}
// 2. Optimized Hash for Noise (Scalar)
fn hash21(p: vec2<f32>) -> f32 {
    let h = hash22(p);
    return h.x;
}
